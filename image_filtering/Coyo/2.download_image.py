...
def sea_vqa_download_images(dataset, col_name='image_path', image_dir='sea_vqa_downloaded_images'):
    ...
    for split in dataset.keys():
        for idx, row in tqdm(enumerate(dataset[split]), total=len(dataset[split])):
            ...
            dataset[split][idx][col_name] = image_filename
...
coyo_dataset = datasets.load_from_disk("ALL_DATA_filtered")
...
coyo_download_images(final_data)
